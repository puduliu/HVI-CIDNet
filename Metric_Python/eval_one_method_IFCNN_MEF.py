...
def calculate_psnr_skimage(img1, img2, gt): # TODO channel_axis= -1 计算的是彩色图
    psnr1 = psnr(img1, gt, data_range=255)
    psnr2 = psnr(img2, gt, data_range=255)
    return 0.5 * psnr1 + 0.5 * psnr2

def calculate_ssim_average(img1, img2, gt): # TODO channel_axis= -1 计算的是彩色图
    """计算图像的平均 SSIM"""
    ...
    ssim2 = ssim(img2, gt, data_range=1)
    return 0.5 * ssim1 + 0.5 * ssim2
...
def evaluation_one(ir_name, vi_name, f_name):
    ...
    f_img_int = np.array(f_img).astype(np.int32) # TF.to_tensor(X), 只有 uint8 会自动除以 255
    ...
    ir_img_double = preprocess_image(ir_img_double)
    vi_img_double = preprocess_image(vi_img_double)
    # PSNR = calculate_psnr_skimage(ir_img_double, vi_img_double, f_img_double)
    # SSIM = calculate_ssim_average(ir_img_double, vi_img_double, f_img_double)
...
if __name__ == '__main__':
    ...
    dataset_name = 'MEFB'
    ir_dir = "/media/zyserver/data16t/lpd/HVI-CIDNet/datasets/inputs/over"
    vi_dir = "/media/zyserver/data16t/lpd/HVI-CIDNet/datasets/inputs/under"
    Method = 'IFCNN'
    ...
    filelist1 = natsorted(os.listdir(ir_dir))
    filelist2 = natsorted(os.listdir(vi_dir))
    filelist3 = natsorted(os.listdir(f_dir))
    # 检查两个列表的长度是否相同
    if len(filelist1) != len(filelist2):
        raise ValueError("ir_dir 和 vi_dir 中的文件数量不匹配")

    eval_bar = tqdm(zip(filelist1, filelist2, filelist3))  # 同时遍历两个文件列表

    for item1, item2, item3 in eval_bar:
        ir_name = os.path.join(ir_dir, item1)
        vi_name = os.path.join(vi_dir, item2)
        f_name = os.path.join(f_dir, item3)
        ...
        filename_list.append(item3)
        eval_bar.set_description("{} | {}".format(Method, item3))
    if with_mean:
    # 添加均值
        ...
        # 写入txt文件，路径和文件名自行调整
        txt_path = os.path.join(save_dir, f'metric_mean_{dataset_name}_{Method}.txt')
