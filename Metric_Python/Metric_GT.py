...
def PSNR_function(A, F): # TODO 别人的matlab代码也是进行了归一化
    A = A / 255.0
    F = F / 255.0
    m, n = F.shape
...
def SCD_function(A, B, F): # SCD 指标的定义本身就是围绕两个源图像来设计的
    r = corr2(F - B, A) + corr2(F - A, B)
    return r

def Qabf_function(A, B, F): # Qabf也是两个两个源图像与融合图像 (F) 的质量指标
    return get_Qabf(A, B, F)

def Nabf_function(A, B, F): # Nabf也是两个两个源图像与融合图像 (F) 的质量指标
...
def SSIM_function(A, F):
    # TODO edit start ssim内部已经做了归一化
    A = A / 255.0
    F = F / 255.0
    # TODO edit end
